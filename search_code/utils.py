...
class RAQueue(Queue):
    """A Ramificacion y Acotacion Queue."""
...
class RASQueue(Queue):
    """A Ramificacion y Acotacion con subestimacion Queue."""
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda n: n.path_cost + self.problem.h(n))
    def pop(self):
        e = self.A[self.start]
        self.start += 1
        if self.start > 5 and self.start > len(self.A) / 2:
